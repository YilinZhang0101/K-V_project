...
int skipList<K, V>::insert_node(K in_key, V in_val) {
    // define a head node
    skipListNode<K, V> *cur = this->skiplist_head;
...
template <typename K, typename V>
void skipList<K, V>::delete_node(K out_key) {
    skipListNode<K, V> *cur = this->skiplist_head;
    skipListNode<K, V> *keep_pre[max_level + 1];
    ...
    for (int i = skipList_level;i >= 0;i --) {
        while (cur->nextPtr[i] != NULL && cur->nextPtr[i]->get_key() < out_key) {
            cur = cur->nextPtr[i];
        }
        keep_pre[i] = cur;
    }
     
    // 'cur->nextPtr[0]' is the node next to cur in lowerest skipList_level
    // we can't get the random_level of 'cur->nextPtr[0]'
    // need to check if each level include it
    // if (cur->nextPtr[0] == NULL || cur->nextPtr[0]->get_key() != out_key) {
    //     return 1;
    // }
    
    if (cur->nextPtr[0] != NULL && cur->nextPtr[0]->get_key() == out_key) {
        // skipListNode<K, V> *target = cur->nextPtr[0];
        skipListNode<K, V> *del = cur->nextPtr[0];
        for (int i = 0;i <= skipList_level;i ++) {
            if (keep_pre[i]->nextPtr[i] != del) {
                break;
                // the target node is not included in this level
            }
            keep_pre[i]->nextPtr[i] = del->nextPtr[i];
        }
         
        // change the level of updated skipList
        for (int i = skipList_level;i > 0;i --) {
            // if one level only has head node
            if (skiplist_head->nextPtr[i] == NULL) {
                skipList_level --;
            }
            else {
                // lower level all have more than one nodes
                break;
            }
        }
         
        // while (skipList_level > 0 && skiplist_head->nextPtr[skipList_level] == NULL) {
        //     skipList_level --;
        // }
...
int main() {
    ...
    for (int i = 0; i < M; i++) {
        int key;
        std::cin >> key;
        // if (skip_list->delete_node(key) == 0) {
